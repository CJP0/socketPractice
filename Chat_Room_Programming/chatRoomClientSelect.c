...
void sendMsgReadStdin(void){
	read(STDIN_FILENO, sendbuf + sizeof(struct msgHeader), MAXMSGSIZE - sizeof(struct msgHeader));
...
int main(int argc, char *argv[]){
	...
	fd_set rdfds, masterfdset;
	FD_ZERO(&rdfds);
	FD_ZERO(&masterfdset);
	FD_SET(STDIN_FILENO, &masterfdset);
	...
	while(1){
		rdfds = masterfdset;
		if ((selectTemp = select(sockfd+1, &rdfds, NULL, NULL, NULL)) == -1) {
			printf("select error\n");
			exit(1);
		}
